...
from .models import Locatie, Tur
...
def detalii_locatie(request, locatie_id):
    ...
    context = {
        ...
        'tururi': tururi,
        'oras': locatie.oras,
        'judet': locatie.oras.judet,

    }
    return render(request, 'detalii_locatie.html', context)
...
def creare_tur_personalizat(request):
    ...
    if request.method == 'POST':
        ids_locatie_selectata = request.POST.get('locatii')
        locatii_selectate = Locatie.objects.get(id__in=ids_locatie_selectata)
